                    ...
                    <div class="nk-block">
                        <div class="card card-bordered card-stretch">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-sm-4">
                                        <h3 class="nk-block-title page-title">Outward Lists</h3>
                                    </div>
                                    <div class="col-md-8 text-right">
                                        <a v-bind:href="create_outward" class="dropdown-toggle btn btn-icon btn-primary mx-2"><em class="icon ni ni-plus"></em></a>
                            ...
                            <div class="card-inner table-responsive">
                                <table class="table table-hover" id="outward">
                                    <thead>
                                        <tr>
                                            <th>Id</th>
                                            <th>OUID</th>
    ...
    export default {
        ...
        methods: {
            ...
            clearallfilter(event) {
                $('#outward_filter').find("input[type=search]").val('');
                $('#outward_filter').find("input[type=date]").val('');
                $('#outward').DataTable().clear().draw();
            },
        },
        mounted() {
            const self = this;
            ...
            function init_dt_table () {
                dt_table = $('#outward').DataTable({
                    processing: true,
                    serverSide: true,
                    lengthChange: true,
                    ajax: {
                        url: "/register/outwardlist",
                        data: function (data) {
                            if ($('#ouid').val() == '') {
                                data.columns[0].search.value = '';
                            } else {
                                data.columns[0].search.value = $('#ouid').val();
                            }
                            if ($('#date_added').val() == '') {
            ...
            var draw = 1;
            $(document).on('keyup', '#outward_filter input', function(e) {
                 if ($(this).val() == '') {
                    if (draw == 0) {
                        dt_table.clear().draw();
                        draw = 1;
