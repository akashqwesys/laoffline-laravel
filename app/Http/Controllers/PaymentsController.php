...
class PaymentsController extends Controller
{
    ...
    {
        return Company::where('is_delete', 0)->where('company_type', 3)->get();
    }

    public function listCustomer()
    {
        return Company::where('is_delete', 0)->where('company_type', 2)->get();
    }
    ...
    public function insertPaymentData(Request $request)
    {
        ...
        try {
            ...
            if ($paymentData->refrence == '1') {
                ...
                $refence->id = $refence->generateAutoId();
            ...
            $iuid_ids = new Iuid();
            $iuid_ids->id = $iuid_ids->generateAutoId();
            $iuid_ids->iuid = $iuid;
            $iuid_ids->financial_year_id = $financialid;
            $iuid_ids->save();

            $combo_id = Comboids::generateAutoId();
            ...
            $paymentId = Payment::generateAutoId();
            ...
            if ($paymentSalebill) {
                ...
                foreach ($paymentSalebill as $salebill) {
                    ...
                    $Id = PaymentDetail::generateAutoId();
    ...
    public function fetchPayment(Request $request, $id)
    {
        ...
        foreach ($paymentDetail as $details) {
            ...
            $salebilldata = array(
                'id' => $details->sr_no,
                'fid' => $details->financial_year_id,
                'sup_inv' => $details->supplier_invoice_no,
                'amount' => $details->amount,
                'adjustamount' => $details->adjust_amount,
                'status' => $status,
                'discount' => $details->discount,
                'discountamount' => $details->discount_amount,
                'goodreturn' => $details->goods_return,
                'ratedifference' => $details->rate_difference,
                'bankcommission' => $details->bank_commission,
                'vatav' => $details->vatav,
                'agentcommission' => $details->agent_commission,
                'claim' => $details->claim,
                'short' => $details->short,
