...
class CompanyController extends Controller
{
    ...
    public function index()
    {
        ...
        $logs->log_path = 'Company / View';
        $logs->log_subject = 'Company view page visited.';
        ...
        try {
            ...
            if (is_array($contactDetails) && !empty($contactDetails)) {
                foreach ($contactDetails as $contactDetail) {
                    if (trim($contactDetail->contact_person_name)) {
                        $companyContactId = CompanyContactDetails::generateAutoId();
                ...
                $companyContactId = CompanyContactDetails::generateAutoId();
                CompanyContactDetails::create([
                    'id' => $companyContactId,
                    'company_id' => $companyId,
                    'contact_person_name' => '',
                    'contact_person_designation' => 0,
            ...
            if (is_array($multipleAddresses) && !empty($multipleAddresses)) {
                foreach ($multipleAddresses as $multipleAddress) {
                    ...
                    $companyAddress->id = CompanyAddress::generateAutoId();
                    ...
                    if (is_array($multipleAddress->multipleAddressesOwners) && !empty($multipleAddress->multipleAddressesOwners)) {
                        foreach ($multipleAddress->multipleAddressesOwners as $owner) {
                            ...
                            $companyAddressOwner->id = CompanyAddressOwner::generateAutoId();
                        ...
                        CompanyAddressOwner::create([
                            'id' => CompanyAddressOwner::generateAutoId(),
                            'company_address_id' => $companyAddress->id,
                            'name' => '',
                            'designation' => 0,
                            'profile_pic' => '',
                            'mobile' => '',
                ...
                $companyAddress = CompanyAddress::create([
                    'id' => CompanyAddress::generateAutoId(),
                    'company_id' => $companyId,
                    'address_type' => 0,
                    'address' => '',
                    'country_code' => '+91',
            ...
            if (is_array($multipleEmails) && !empty($multipleEmails)) {
                ...
                CompanyEmails::create([
                    'id' => CompanyEmails::generateAutoId(),
                    'company_id' => $companyId,
                    'email_id' => '',
                ]);
            }
            ...
            $swotData->id = CompanySwotDetails::generateAutoId();
            $swotData->company_id = $companyId;
            $swotData->strength = $swotDetails->strength ?: '0';
            $swotData->weakness = $swotDetails->weakness ?: '0';
            $swotData->opportunity = $swotDetails->opportunity ?: '0';
            $swotData->threat = $swotDetails->threat ?: '0';
            $swotData->save();

            // Bank Data
            $bankDetail = new CompanyBankDetails;
            $bankDetail->id = CompanyBankDetails::generateAutoId();
            ...
            $package->id = CompanyPackagingDetails::generateAutoId();
            $package->company_id = $companyId;
            $package->gst_no = $packagingDetails->gst_no ?? '0';
            $package->cst_no = $packagingDetails->cst_no ?? '0';
            $package->tin_no = $packagingDetails->tin_no ?? '0';
            $package->vat_no = $packagingDetails->vat_no ?? '0';
            $package->save();

            // Reference Data
            $reference = new CompanyReferences;
            $reference->id = CompanyReferences::generateAutoId();
