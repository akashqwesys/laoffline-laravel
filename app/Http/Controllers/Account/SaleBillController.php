...
class SaleBillController extends Controller
{
    ...
    public function addSaleBill(Request $request)
    {
        ...
        // Validate that supplier_invoice_no is unique
        if (!empty($referenceDetails->supplier_invoice_no) && $supplier_id) {
            $existingBill = DB::table('sale_bills')
                ->where('is_deleted', 0)
                ->where('supplier_id', $supplier_id)
                ->where('supplier_invoice_no', $referenceDetails->supplier_invoice_no)
                ->where('financial_year_id', $user->financial_year_id)
                ->first();
            ...
            if ($existingBill) {
                return response()->json([
                    'success' => false,
                    'message' => 'Supplier Invoice Number already exists for this supplier in the current financial year.'
                ], 422);
            }
        }
